        ...
        <el-form label-width="100px">
          ...
          <div>
            <div v-for="(item, index) in form.questions" :key="item.key">
              <h4>{{index + 1}}. {{item.title}} <el-button @click="openEditDialog(item, index)" icon="el-icon-edit" type="primary" circle></el-button></h4>
      ...
      <el-dialog width="640px" title="编辑问题" :visible.sync="showEditDialog">
        <el-form>
          ...
          <el-form-item style="margin-bottom: 0;">
            <div v-for="(option, index) in currentQuestion.options" :key="option.key" style="margin-bottom: 14px;">
              <el-input style="width: 240px;" v-model="currentQuestion.options[index]"></el-input>
              <el-button type="danger" icon="el-icon-delete" @click="currentQuestion.options.splice(index, 1)"></el-button>
            </div>
          </el-form-item>
          <el-form-item>
            <el-input style="width: 240px;" v-model="optionBuf"></el-input>
            <el-button icon="el-icon-plus" @click="currentQuestion.options.push(optionBuf); optionBuf = ''">添加选项</el-button>
          </el-form-item>
        </el-form>
  ...
  methods: {
    ...
    saveQuestion () {
      if (this.editMode === 'add') {
        this.form.questions.push(JSON.parse(JSON.stringify(this.currentQuestion)))
      } else {
        this.form.questions.splice(this.currentIndex, 1, JSON.parse(JSON.stringify(this.currentQuestion)))
      }
    ...
    save () {
      if (this.form.id) {
        ...
        axios.post('/survey/create', this.form).then(() => {
